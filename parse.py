...
output_filename = os.getenv("OUTPUT_FILENAME", "CIS_RCP_export.xlsx")
...
def _find_input_file() -> Path:
    ...
    # 1. Vérifier .env d'abord
    env_path = os.getenv("INPUT_FILE_PATH")
    if env_path:
        p = Path(env_path)
        if p.exists():
            print(f"✅ Fichier trouvé via INPUT_FILE_PATH: {p}")
            return p
        else:
            print(f"⚠️  INPUT_FILE_PATH spécifié mais fichier non trouvé: {p}")
    
    # 2. Chercher localement (ancien comportement)
    for name in ("CIS_RCP.html", "CIS_RCP.htm"):
        p = Path(name)
        if p.exists():
            print(f"✅ Fichier trouvé localement: {p}")
            return p
    
    # 3. Valeur par défaut pour message d'erreur
    return Path("CIS_RCP.html")
